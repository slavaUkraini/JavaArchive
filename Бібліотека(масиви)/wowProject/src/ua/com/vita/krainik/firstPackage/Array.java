...
public class Array {
	...
	public void sortZrost(){
		//Сортування за зростанням vstavka
		for(int i=0; i<mass.length;i++){
	...
	public int getElement(int number){
		//Функція повертає елемент за номером
	...
	void printmass(int n) {
		/*Вивід перших n елементів масиву на екран*/
		if(n<=mass.length){
			for (int i = 0; i < n; i++){
	...
	int getNumber (int number){// поки що не використовується
